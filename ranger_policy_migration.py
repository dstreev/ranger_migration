...
def main():
    ...
    connect_group.add_option('-u', '--ranger-host-url', dest='host_url', help='Ranger Host Base URL(required)')
    connect_group.add_option('-n', '--username', dest='user', help='Username(required)')
    connect_group.add_option('-p', '--password', dest='password', help='Password(required)')
    ...
    service_group.add_option('-f', '--from-service', dest='service_from',
                      help='Ranger service "from" Name(required)')
    service_group.add_option('-t', '--to-service', dest='service_to', help='Ranger service "to" Name(required)')
    ...
    if options.host_url is None or options.user is None or options.password is None or options.service_from is None\
            or options.service_to is None:
        print ('Required inputs missing: -u, -n, -p, -f, and -t are required options.')
        exit(-1)
